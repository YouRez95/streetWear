...
export default function Login() {
  ...
  const handleLoginUser: React.FormEventHandler<HTMLFormElement> = async (e) => {
    e.preventDefault()
    if (!email || !password) {
      setError('Please fill in all fields.')
      return
    }
    const response = await window.context.loginUser({ email, password })
    ...
    if (response.status === 'success' && response.user) {
      setError('')
      // await window.context.setToStore('user', response.user)
    ...
    <section className="flex bg-muted h-screen relative">
      ...
      <div className="flex-1 flex justify-center">
        <div className="flex flex-col gap-5 items-center w-[60%] max-w-[800px] justify-center bg-muted my-auto p-10 rounded-xl">
          ...
          <form className="mt-4 flex flex-col w-full gap-4" onSubmit={handleLoginUser}>
            <input
              type="text"
              placeholder="Email"
            ...
            />
            <button type="submit" className="bg-secondary text-white p-2 rounded mt-4 font-bagel">
              Login
            </button>
          </form>
