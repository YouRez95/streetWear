...
import { Check, Clock, Coins, HandCoins, Info, Pencil, Trash, Undo2 } from 'lucide-react'
...
export const WorkerRecordsTable = ({
  ...
  if (!workerId) {
    return (
      <div className="text-background/50 text-center my-10">Veuillez sélectionner un employé</div>
    )
  }
  ...
  const calculateRecordValues = (rec: any) => {
    ...
    const extraHours =
      ...
      rec.vendrediSupp +
      rec.samediSupp
    const normalHours = rec.lundi + rec.mardi + rec.mercredi + rec.jeudi + rec.vendredi + rec.samedi
  ...
  return (
    <>
        ...
        <Table className="w-full border-collapse text-base">
          <TableHeader>
            <TableRow className="border border-background/35 divide-x divide-background/35">
              <TableHead className="text-background/65 min-w-[160px]">Semaine</TableHead>
              <TableHead className="text-background/65 min-w-[120px]">Atelier</TableHead>
          ...
          <TableBody>
            {records.length === 0 ? (
              <TableRow>
                <TableCell colSpan={17} className="text-center p-4 text-background/65">
              ...
              records.map((rec) => {
                ...
                return (
                  <TableRow
                    key={rec.id}
                    className={`border border-background/35 divide-x divide-background/35 hover:bg-background/5 transition-colors ${rec.isPaid ? '' : 'bg-destructive/15'}`}
                  >
                    <TableCell className="font-medium text-sm">{rec.displayText}</TableCell>
                    <TableCell className="font-medium">{rec.workplace.name}</TableCell>
                    ...
                    {[rec.lundi, rec.mardi, rec.mercredi, rec.jeudi, rec.vendredi, rec.samedi].map(
                      (v, i) => (
                        <TableCell key={i} className="text-center py-3 px-2 min-w-[50px]">
                          <span className="inline-block min-w-[30px] font-mono">{v}</span>
                        </TableCell>
                      )
                    )}
                    ...
                    <TableCell className="text-right font-mono">{rec.avance} dh</TableCell>
      ...
      <div className="space-y-4 block md:hidden">
        ...
        {records.map((rec) => {
          const { extraHours, totalHours, totalSalaire, reste } = calculateRecordValues(rec)

          return (
            <div
              key={rec.id}
              className={`border rounded-lg overflow-hidden ${
                rec.isPaid ? 'border-background/20' : 'border-destructive/30 bg-destructive/5'
              }`}
            >
              {/* Header */}
              <div className="bg-muted-foreground p-4 flex justify-between items-start">
                <div>
                  <h3 className="font-semibold text-lg text-background">{rec.worker.name}</h3>
              ...
              <div className="p-4 bg-background/5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    >
                      <div className="text-xs text-background/60 font-medium">{day}</div>
                      <div className="text-sm font-semibold text-background">
                        {hours}h
                        {supp > 0 && <span className="text-xs text-orange-600"> +{supp}</span>}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                <div className="bg-blue-50 border border-blue-200 rounded p-3">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-blue-600 font-medium">Total heures</span>
                  </div>
                  <p className="text-lg font-bold text-blue-900">{totalHours}h</p>
                  {extraHours > 0 && <p className="text-xs text-blue-600">+{extraHours}h supp.</p>}
                </div>

                <div className="bg-green-50 border border-green-200 rounded p-3">
                  <div className="flex items-center gap-2 mb-1">
                    <Coins className="w-4 h-4 text-green-600" />
                    <span className="text-xs text-green-600 font-medium">Salaire total</span>
                  </div>
                  <p className="text-lg font-bold text-green-900">{totalSalaire.toFixed(0)} dh</p>
                </div>

                <div className="bg-orange-50 border border-orange-200 rounded p-3">
                  <div className="flex items-center gap-2 mb-1">
                    <HandCoins className="w-4 h-4 text-orange-600" />
                    <span className="text-xs text-orange-600 font-medium">Avance</span>
                  </div>
                  <p className="text-lg font-bold text-orange-900">{rec.avance} dh</p>
                </div>

                <div className="bg-purple-50 border border-purple-200 rounded p-3">
                  <div className="flex items-center gap-2 mb-1">
                    <Coins className="w-4 h-4 text-purple-600" />
                    <span className="text-xs text-purple-600 font-medium">Reste</span>
                  </div>
              ...
              {rec.description && (
                <div className="px-4 pb-4">
                  <div className="bg-background/10 border border-background/20 rounded p-3">
                    <p className="text-xs font-medium text-background/60 mb-1">Note</p>
                    <p className="text-sm text-background/80">{rec.description}</p>
                  </div>
                </div>
              )}
