...
import { useYearRecords } from '@renderer/hooks/useWorkers'
import { useYearStore } from '@renderer/store'
...
export default function YearSummary({ setTotalYear, setNextYear, setPrevYear }: YearSummaryProps) {
  const { workplaceId, year } = useYearStore()
  const { data, isPending, isError } = useYearRecords(year, workplaceId)

  useEffect(() => {
    if (data && data.records) {
      const total = data.records.reduce((acc, month) => acc + month.totalAmount, 0)
  ...
  if (data.status === 'failed') {
    return (
      <div className="text-center text-red-500">Erreur : {data.message || 'Something wrong'}</div>
    )
  }

  if (data && data.records && data.records.length === 0) {
    return (
      <div className="text-center text-gray-500">Aucun enregistrement trouvé pour cette année.</div>
    )
  }
...
export function CardMonth({ month }: { month: Month }) {
  return (
    <Card className="w-full max-w-sm shadow-lg rounded-lg hover:shadow-xl transition-shadow duration-300 flex flex-col justify-between">
        ...
        <div className="flex flex-col gap-1">
          {month.weeks.map((week, index) => (
            ...
            >
              ...
              <span className="font-medium text-gray-900">{week.totalAmount.toFixed(0)}</span>
            </div>
          ))}
        </div>
      </CardContent>
      ...
      <CardFooter className="flex justify-between items-center p-4 border-t">
        ...
        <span className="text-lg font-bold text-gray-900">{month.totalAmount.toFixed(0)}</span>
      </CardFooter>
    </Card>
  )
}
