...
import { useWeeksByCursor } from '@renderer/hooks/useWorkers'
...
export default function SelectWeekFilter({
  ...
  const { workplaceId, weekId } = useWorkerStore()
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useWeeksByCursor(
    workplaceId,
    20,
    debouncedSearchTerm
  )
  ...
  return (
    <div className="w-64">
      ...
      <Select onValueChange={handleWeekSelection} value={value} disabled={disabled}>
        ...
        >
          ...
          {status === 'success' && weeks.length === 0 && (
            <div className="p-4 text-center text-background/45">Aucune semaine trouvée</div>
          )}
