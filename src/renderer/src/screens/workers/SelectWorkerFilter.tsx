...
import { useWorkersByCursor } from '@renderer/hooks/useWorkers'
...
export default function SelectWorkerFilter({
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useWorkersByCursor(
    20,
    debouncedSearchTerm
  )
      ...
      <Select onValueChange={onValueChange} value={value} disabled={disabled}>
        ...
        >
          ...
          {status === 'success' && workers.length === 0 && (
            <div className="p-4 text-center text-background/45">Aucune employé trouvée</div>
          )}
