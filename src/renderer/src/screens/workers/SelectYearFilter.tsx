...
import { useYearsByCursor } from '@renderer/hooks/useWorkers'
...
export default function SelectYearFilter({
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useYearsByCursor(
    workplaceId,
    20,
    debouncedSearchTerm
  )
      ...
      <Select onValueChange={onValueChange} value={value} disabled={disabled}>
        ...
        >
          ...
          {status === 'success' && years.length === 0 && (
            <div className="p-4 text-center text-background/45">Aucune année trouvée</div>
          )}
