...
export default function SelectWorkplaceFilter({
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useWorkPlacesByCursor(
    20,
    debouncedSearchTerm
  )
      ...
      <Select onValueChange={onValueChange} value={value} disabled={disabled}>
        ...
        >
          ...
          {status === 'success' && workplaces.length === 0 && (
            <div className="p-4 text-center text-background/45">Aucun atelier trouvé</div>
          )}
          ...
          {!hasNextPage && workplaces.length > 0 && (
            <div className="p-2 text-center text-xs text-background/45 border-t">
              {workplaces.length} atelier{workplaces.length !== 1 ? 's' : ''} chargé
