...
import { useDeleteProduct } from '@renderer/hooks/useProduct'
...
export function DeleteProductDialog({ product, open, setOpen }: DeleteProductDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-[525px] bg-foreground">
        <DialogHeader>
          <DialogTitle className="text-xl">Supprimer le produit: {product.name}</DialogTitle>
          <DialogDescription className="text-background text-base">
            Êtes-vous sûr de vouloir supprimer ce produit? Cette action ne peut être annulée.
          </DialogDescription>
        </DialogHeader>
