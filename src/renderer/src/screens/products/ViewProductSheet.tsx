  ...
  return (
    <Sheet open={openSheet} onOpenChange={setOpenSheet}>
      <SheetContent className="bg-foreground border min-w-[500px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="text-background text-2xl font-bold">Détails du produit</SheetTitle>
        ...
        <div className="mt-6 flex flex-col gap-6 text-background text-base pb-6">
              ...
              <CardContent className="overflow-x-auto p-0">
                <table className="min-w-full text-sm border-t border-border">
                  <thead className="bg-muted text-left">
                    <tr>
                      {['Styliste', 'Type', 'Bon', 'Date', 'Qté', 'Prix unitaire', 'Total'].map(
                        (head) => (
                          <th key={head} className="px-4 py-2 font-medium text-background/80">
                            {head}
                          </th>
                        )
                      )}
                  ...
                  <tbody>
                    {product.StyleTraitOrderItems.map((item) => (
                      <tr key={item.id} className="border-t border-border">
                        <td className="px-4 py-2">{item.styleTraitOrder.styleTrait.name}</td>
                        ...
                        <td className="px-4 py-2 text-center">{item.quantity_sent}</td>
                        <td className="px-4 py-2 text-center">{item.unit_price.toFixed(2)} dh</td>
          ...
          {product.ProductStatus && (
            <Card>
              ...
              <CardContent className="space-y-2">
                <StatusLine label="Quantité en stock" value={product.ProductStatus.raw_in_stock} />
                <StatusLine
                  label="Au façonnier"
