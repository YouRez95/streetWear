...
export default function TransferMultipleProductsToClient({
  ...
  const [transferDate, setTransferDate] = useState({ date: new Date().toISOString() })
  ...
  const [selectedBonNumber, setSelectedBonNumber] = useState<number | null>(null)

  const [productsToTransfer, setProductsToTransfer] = useState<ProductToTransferType[]>([])
  ...
  const { mutate: createMultipleOrdersClient } = useCreateMultipleOrdersClient()
  ...
  const toggleProductToTransfer = (product: GetProductsResponse['products'][0]) => {
  ...
  const updateQuantity = (index: number, quantity: number) => {
    setProductsToTransfer((prev) => {
      const updated = [...prev]
      updated[index].quantity = Math.min(Math.max(1, quantity), updated[index].quantityReady)
      return updated
    })
  }

  const handleSubmitTransfer = () => {
    if (!selectedClient || !selectedBonNumber || productsToTransfer.length === 0) return
      ...
      <DialogContent className="max-w-3xl bg-foreground overflow-y-auto max-h-[90vh]">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-background/60">
            Select a client and a bon number, then search and choose the products to transfer.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex items-center flex-1 gap-2">
            ...
            <div className="relative flex-1">
              ...
              >
                ...
                <ChevronDown className={`h-4 w-4 ${clientDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              {clientDropdownOpen && (
                  ...
                  {clientsData?.clients?.map((client) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        setSelectedBonNumber(client.BonsClients[0]?.bon_number ?? null)
                        setClientDropdownOpen(false)
                      }}
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-full">
              <button
                ...
                className="w-full text-left border border-background/30 p-2 rounded-md bg-foreground flex justify-between items-center text-sm"
              >
                {selectedBonNumber ? `Bon #${selectedBonNumber}` : 'Select bon number'}
                <ChevronDown className={`h-4 w-4 ${bonDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              {bonDropdownOpen && (
                <div className="absolute z-50 w-full bg-foreground border mt-1 rounded shadow max-h-60 overflow-y-auto">
          ...
          <div className="divide-y mt-2 border border-background/10 rounded overflow-hidden">
            {productsData?.products?.length === 0 ? (
              ...
              productsData?.products
                ...
                .map((product) => {
                  const isSelected = productsToTransfer.some((p) => p.id === product.id)
                  return (
                    <div
                      key={product.id}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-background/50">Ref: {product.reference}</p>
                        </div>
                      </div>
                      <p className="text-sm">Ready: {product.ProductStatus.quantity_ready}</p>
                    </div>
                  )
                })
            )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-1">
                    <Input
                      ...
                      className="w-20"
                    />
                    <span className="text-xs text-background/40">/ {product.quantityReady}</span>
                  </div>
                  ...
                  <Button
                    ...
                    variant="ghost"
                    onClick={() =>
                      setProductsToTransfer((prev) => prev.filter((p) => p.id !== product.id))
                    }
                  >
