...
import { useDeleteBonStylist } from '@renderer/hooks/useStylist'
import { cn } from '@renderer/lib/utils'
import { useState } from 'react'

type DeleteBonStylistDialogProps = {
  open: boolean
...
export const DeleteBonStylistDialog = ({
  open,
  setOpen,
  bonNumber,
  bonId
}: DeleteBonStylistDialogProps) => {
  const { mutate: deleteBonMutation } = useDeleteBonStylist()
  ...
  return (
    ...
    >
      <DialogContent className="bg-foreground rounded-xl p-5 shadow-sm border space-y-3">
        <DialogHeader>
          <DialogTitle className="text-xl">Delete Bon #{bonNumber}</DialogTitle>
          <DialogDescription className="text-background text-base">
            Êtes-vous sûr de vouloir supprimer ce bon ? Cette action ne peut être annulée.
          </DialogDescription>
        </DialogHeader>
