...
import { useDeleteOrderStylist } from '@renderer/hooks/useStylist'
...
export const DeleteOrderStylistDialog = ({
  ...
  const { open, orderId, reference } = openDeleteOrderDialog
  ...
  return (
    ...
    >
      <DialogContent className="bg-foreground rounded-xl p-5 shadow-sm border space-y-3">
        <DialogHeader>
          <DialogTitle className="text-xl">Supprimer la commande: {reference}</DialogTitle>
          <DialogDescription className="text-background text-base">
            Êtes-vous sûr de vouloir supprimer cette commande ? Cette action ne peut être annulée.
          </DialogDescription>
        </DialogHeader>
