...
import { useRetardOrdersFaconnier } from '@renderer/hooks/useDashboard'
import { useCancelOrderFaconnier } from '@renderer/hooks/useFaconnier'
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl sm:text-2xl font-bold font-bagel">Commandes en retard</h2>
      </div>

      {/* Desktop Table View - Hidden on Mobile */}
      <div className="hidden lg:block">
        <Table className="border-background/10 rounded-xl text-base border">
          <TableCaption className="sr-only">Liste des commandes en retard</TableCaption>
          <TableHeader className="text-background bg-tableHead border">
            <TableRow>
              ...
              <TableHead className="text-background text-center">Quantité attendue</TableHead>
              <TableHead className="text-background text-center">Quantité retournée</TableHead>
              <TableHead className="text-center text-background">Retard (jours)</TableHead>
              <TableHead className="text-right text-background">Action</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody className="text-base">
            {retardOrdersFaconnier?.faconniers.map((faconnier) => (
              <TableRow key={faconnier.id} className="h-[55px] hover:shadow-sm transition-all">
                <TableCell className="font-medium border-l">{faconnier.reference}</TableCell>
                ...
                <TableCell className="text-center">
                  {faconnier.quantityExpected} {faconnier.quantityExpected > 1 ? 'pcs' : 'pc'}
                </TableCell>
                <TableCell className="text-center">
                  {faconnier.quantityReturned} {faconnier.quantityReturned > 1 ? 'pcs' : 'pc'}
                </TableCell>
                <TableCell className="text-center">
      ...
      <div className="lg:hidden space-y-4">
          ...
          >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-background/50 mt-1">Réf: {faconnier.reference}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="font-medium truncate">{faconnier.faconnierName}</p>
                </div>
              </div>
        ...
        {(!retardOrdersFaconnier?.faconniers || retardOrdersFaconnier.faconniers.length === 0) && (
