...
import { useDeleteClientReturnStock } from '@renderer/hooks/useReturnStock'
    ...
    deleteStockReturnMutation.mutate(stockReturn.id, {
      onSuccess: (data) => {
        if (data.status === 'failed') {
          setError(data.message || 'Une erreur est survenue lors de la suppression du retour')
          return
        }
        setCode('')
        setOpen(false)
      ...
      <DialogContent className="sm:max-w-[700px] bg-foreground">
        <DialogHeader>
          ...
          <DialogDescription className="text-background text-base space-y-2">
            ...
            <strong>
              {stockReturn.client ? stockReturn.client.name : stockReturn.passagerName}{' '}
            </strong>{' '}
            le <strong>{formatDateToDDMMYYYY(new Date(stockReturn.date))}</strong> ?<br />
            <br />
            ...
            <br />
            <br />
            <strong>Conséquence :</strong> Cette action est irréversible. La quantité retournée sera
            supprimée de l’historique et réaffectée comme disponible chez le client.
          </DialogDescription>
        </DialogHeader>
