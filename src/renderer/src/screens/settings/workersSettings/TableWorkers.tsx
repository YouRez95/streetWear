...
import { useWorkers } from '@renderer/hooks/useWorkers'
...
export default function TableWorker({ searchTerm, active, workplaceId }: TableWorkerProps) {
  ...
  const {
    ...
    error,
    isFetching
  } = useWorkers(active, page, limit, workplaceId, debouncedSearchTerm)
  ...
  const openDialog = (worker: GetWorkersResponse, type: 'update' | 'delete') => {
  ...
  return (
    <>
      ...
      {!isLoading && !error && workersResponse && workersResponse.workers.length === 0 && (
        <p className="text-background/50">Aucuns employés trouvés</p>
      )}

      {!isLoading && !error && workersResponse && workersResponse.workers.length > 0 && (
        <div className="p-7 w-full h-full">
          <LazyTableWorkers openDialog={openDialog} workers={workersResponse.workers} />
        </div>
      )}

      {open && selectedWorker && dialogType === 'update' && (
        <UpdateWorkerDialog worker={selectedWorker} open={open} closeDialog={closeDialog} />
      )}
