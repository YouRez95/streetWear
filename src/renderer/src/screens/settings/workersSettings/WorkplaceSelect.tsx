...
import { useWorkPlaces } from '@renderer/hooks/useWorkers'
...
export default function WorkplaceSelect({
  ...
  useEffect(() => {
    ...
    setAllWorkplaces((prev) => {
      ...
      const newWorkplaces = workPlacesResponse.workplaces.filter((wp) => !existingIds.has(wp.id))
  ...
  const handleScroll = useCallback(() => {
    ...
    const hasMorePages = workPlacesResponse.currentPage < workPlacesResponse.totalPages
      ...
      <div className="col-span-3 space-y-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            className={`
              ...
              ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-background/50'}
            `}
        ...
        >
          ...
          <SelectContent className="max-h-60">
            <div ref={scrollRef} onScroll={handleScroll} className="max-h-60 overflow-auto">
              ...
              {!isFetching &&
                ...
                workPlacesResponse.totalPages > 1 && (
                  <div className="px-2 py-1 text-xs text-background/50 text-center border-t">
                    {allWorkplaces.length} lieu{allWorkplaces.length > 1 ? 'x' : ''} trouvé
        ...
        {/* Helper text */}
        {hasUserInteracted && searchTerm && !isLoading && allWorkplaces.length > 0 && (
          <p className="text-xs text-background/60">
            {allWorkplaces.length} résultat{allWorkplaces.length > 1 ? 's' : ''} trouvé
            {allWorkplaces.length > 1 ? 's' : ''}
