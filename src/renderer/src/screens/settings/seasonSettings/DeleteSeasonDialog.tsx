...
import { useDeleteSeason } from '@renderer/hooks/useSeason'
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && closeDialog()}>
      <DialogContent className="sm:max-w-[525px] bg-foreground">
        <DialogHeader>
          <DialogTitle className="text-xl">Supprimer la saison: {seasonName}</DialogTitle>
          <DialogDescription className="text-background text-base">
            Êtes-vous sûr de vouloir supprimer cette saison ? Cette action ne peut être annulée.
          </DialogDescription>
        </DialogHeader>
