...
export default function TableSeason({ searchTerm }: TableSeasonProps) {
  ...
  const [dialogType, setDialogType] = useState<'update' | 'delete' | 'toggle' | null>(null)
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {seasonsData && seasonsData.seasons && (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 w-full p-7">
          {seasonsData.seasons.map((season, index) => (
            <Card
              key={season.id}
              className="w-full h-fit rounded-xl overflow-hidden hover:shadow-xl transition-all cursor-pointer"
            >
              <CardHeader
                className={cn(
                  `relative space-y-4 h-[180px] flex flex-col justify-between`,
                  index % 2 === 0 ? 'bg-background/80' : 'bg-secondary/80'
                )}
              >
                <CardTitle className="text-lg font-bagel text-foreground flex items-center">
                  <span className="truncate mr-2">{season.name.toUpperCase()}</span>
                  <Switch
                ...
                <CardDescription className="text-base font-medium text-foreground/70 space-y-2">
                  <p className="line-clamp-2">
                    {season.description ? season.description : 'Aucune description trouvée'}
                  </p>
                  {season.summary.type && (
                ...
                {userData?.role === 'super admin' && (
                  <Popover>
                    <PopoverTrigger asChild className="absolute right-2 -top-2 gap-2 flex">
                      <Button
                        variant="ghost"
                    ...
                    <PopoverContent align="end" className="flex flex-col w-fit min-w-52 p-0">
                      <Button
                        variant={'ghost'}
              ...
              <CardContent className="p-0">
                ...
                >
                  <p className="text-xs">{season.isClosed ? 'Saison fermée' : 'Saison ouverte'}</p>
                </div>
              </CardContent>
            </Card>
          ))}
