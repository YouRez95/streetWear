...
function LazyTableWorkplaces({ workplaces, openDialog }: LazyTableWorkplacesProps) {
      ...
      <TableHeader className="text-background bg-tableHead border">
        <TableRow className="text-background rounded-xl font-medium">
          ...
          {userData?.role === 'super admin' && (
            <TableHead className=" text-background text-right">Actions</TableHead>
          )}
        </TableRow>
      </TableHeader>
      <TableBody className="border">
        {workplaces.map((workplace, index) => (
          <TableRow key={workplace.name} className="h-[55px]">
            ...
            <TableCell className="">{formatDateToDDMMYYYY(workplace.createdAt)}</TableCell>
