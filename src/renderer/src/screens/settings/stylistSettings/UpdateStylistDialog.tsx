...
import { useUpdateStylist } from '@renderer/hooks/useStylist'
import { cn } from '@renderer/lib/utils'
import { removeEmptyValues } from '@renderer/utils'
import { Check, ChevronDown } from 'lucide-react'
import { useEffect, useState } from 'react'
import { StylistType } from './CreateStylistDialog'
...
export function UpdateStylistDialog({ stylist, open, closeDialog }: UpdateStylistDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateStylistMutation.mutate(stylistData, {
      ...
      onError: (error) => {
        setError(error.message || 'Une erreur est survenue lors de la mise à jour du styliste')
      }
    })
  }
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 w-full bg-foreground border border-background/50 rounded-md mt-1 z-50">
                  {options.map((option) => (
                    ...
                    >
                      {option}
                      {formData.type === option && <Check className="h-4 w-4" />}
                    </div>
                  ))}
                </div>
              )}
