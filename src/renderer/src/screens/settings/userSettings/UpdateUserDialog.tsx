...
import { useUpdateUser } from '@renderer/hooks/useUsers'
import { getImageUrl, removeEmptyValues, validateUserForm } from '@renderer/utils'
...
export function UpdateUserDialog({ user, open, closeDialog }: UpdateUserDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const errorMessage = validateUserForm({ ...formData, skipPasswordValidation: true })
    if (errorMessage) {
      ...
      <DialogContent className="sm:max-w-[750px] bg-foreground text-base">
        <DialogHeader>
          ...
          <DialogDescription className="text-background/80">
            Modifier les informations et les rôles de l'utilisateur. Laissez les champs de mot de
            passe vides pour conserver le mot de passe actuel.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            >
              <SelectTrigger className="col-span-3 data-[placeholder]:text-background border-background/35">
