...
import { useDeleteUser } from '@renderer/hooks/useUsers'
...
export function DeleteUserDialog({ userId, userName, open, closeDialog }: DeleteUserDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && closeDialog()}>
      <DialogContent className="sm:max-w-[525px] bg-foreground">
        <DialogHeader>
          <DialogTitle className="text-xl">Supprimer l'utilisateur: {userName}</DialogTitle>
          <DialogDescription className="text-background text-base">
            Êtes-vous sûr de vouloir supprimer cet utilisateur ? Cette action ne peut être annulée.
          </DialogDescription>
        </DialogHeader>
