...
export const ToggleActivation = <TData, TError = Error, TVariables = any, TContext = unknown>({
  id,
  active,
  mutateFn,
  idField = 'clientId'
  ...
  return (
    <Select value={status} onValueChange={handleChangeStatus}>
      ...
      >
        <span className="text-sm font-bold">{status === 'active' ? 'Actif' : 'Inactif'}</span>
      </SelectTrigger>
      <SelectContent>
        <SelectItem value="active" className="hover:bg-success/10">
          Actif
        </SelectItem>
