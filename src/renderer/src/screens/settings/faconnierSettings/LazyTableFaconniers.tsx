...
import { useUpdateFaconnierStatus } from '@renderer/hooks/useFaconnier'
...
function LazyTableFaconniers({ faconniers, openDialog }: LazyTableFaconniersProps) {
    ...
    <Table className="border-background rounded-xl overflow-hidden text-base">
      ...
      <TableHeader className="text-background bg-tableHead border">
        <TableRow className="text-background rounded-xl font-medium">
          ...
          <TableHead className="text-background text-center">Actions</TableHead>
        </TableRow>
      </TableHeader>
        ...
        {faconniers.map((faconnier, index) => (
          <TableRow key={faconnier.name} className="h-[55px]">
            ...
            <TableCell className="h-full">
              <div className="flex items-center gap-2 h-full">{faconnier.name}</div>
            </TableCell>
            <TableCell className="">{formatDateToDDMMYYYY(faconnier.createdAt)}</TableCell>
