...
import { useDeleteFaconnier } from '@renderer/hooks/useFaconnier'
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && closeDialog()}>
      <DialogContent className="sm:max-w-[525px] bg-foreground">
        <DialogHeader>
          <DialogTitle className="text-xl">Supprimer le faconnier: {faconnierName}</DialogTitle>
          <DialogDescription className="text-background text-base">
            Êtes-vous sûr de vouloir supprimer ce faconnier ? Cette action ne peut être annulée.
          </DialogDescription>
        </DialogHeader>
