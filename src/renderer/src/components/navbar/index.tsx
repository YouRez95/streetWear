...
import { navbarItems } from '@renderer/data'
...
export const Navbar = () => {
  ...
  async function handleLogout() {
    // TODO: add sonner
    const { status, message } = await window.context.logoutUser()
    if (status === 'success') {
      setLogout()
    }
  }
    ...
    >
      ...
      >
        <img
          src={doubleArrowIcon}
          alt="double arrow"
          className={cn('w-3 h-3 transition-all duration-300', !isOpen && 'rotate-180')}
        />
      </div>
      <div className="flex flex-col justify-between h-full">
        ...
        <div className="space-y-10 m-3">
          ...
          <div className="flex items-center justify-center">
            ...
            <h1 className={cn('font-bagel text-2xl', !isOpen && 'hidden')}>StreetWear</h1>
          </div>
          {/* user */}
          <div
            className={cn(
          ...
          >
            <div>
              <img
                src={userProfileImg}
                alt="Logo"
                className={cn('w-12 h-12 rounded-xl object-cover', !isOpen && 'w-8 h-8')}
                onError={(e) => {
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        ...
                        <span className={cn('', !isOpen && 'hidden')}>{item.text}</span>
                      </>
                    )}
                  </NavLink>
                )
