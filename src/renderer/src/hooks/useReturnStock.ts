import { returnStockService } from '@renderer/services/returnStockService'
...
export const queryKeys = {
  returnStock: (page: number, limit: number, search: string, seasonId: string) => [
    'stock-return',
    seasonId,
    page,
    limit,
...
export function useDeleteClientReturnStock() {
  ...
  return useMutation({
    ...
    onSettled: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['ordersClient', seasonId, data?.data.clientId, data?.data.bonId]
      })
    }
  })
}
...
export function useUpdateClientReturnStock() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      newQuantity: number
    }) => returnStockService.updateClientReturnStock({ seasonId, clientReturnId, newQuantity }),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
...
export function useCreateOrderClientFromReturnStock() {
  ...
  return useMutation({
    ...
    onSettled: (_data, _error, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['ordersClient', seasonId, _data?.data?.clientId, _data?.data?.bonId]
      })
      queryClient.invalidateQueries({
        queryKey: ['clientSummary', seasonId, _data?.data?.clientId, , _data?.data?.bonId]
      })
      queryClient.invalidateQueries({
