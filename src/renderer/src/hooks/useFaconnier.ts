...
export function useDeleteBonFaconnier() {
  ...
  return useMutation({
    mutationFn: (bonId: string) => faconnierService.deleteBonFaconnier(bonId, seasonId),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
...
export function useOrdersFaconnier(faconnierId: string, bonId: string, queryParams: QueryParams) {
  const { activeSeason } = useUserStore()
  const seasonId = activeSeason?.id || ''

  // Always call useQuery, but disable it if faconnierId or bonId is empty
  return useQuery({
    queryKey: queryKeys.ordersFaconnier(seasonId, faconnierId, bonId, queryParams),
    queryFn: () => faconnierService.getOrdersFaconnier(seasonId, faconnierId, bonId, queryParams),
    refetchOnWindowFocus: false,
    retry: false,
...
export function useUpdateOrderFaconnier() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      faconnierService.updateOrderFaconnier({ bonId, faconnierId, orderId, formData, seasonId }),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
      ...
      toast({
        ...
        description: data.message || 'Order faconnier has been updated successfully.',
        variant: 'default'
      })
    },
...
export function useCancelOrderFaconnier() {
  ...
  return useMutation({
    mutationFn: (orderId: string) => faconnierService.cancelOrderFaconnier(orderId),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
      ...
      toast({
        ...
        description: data.message || 'Order faconnier has been canceled successfully.',
        variant: 'default'
      })
    },
    onError: (error) => showErrorToast('Error canceling order faconnier', error),
    onSettled: () => {
      queryClient.invalidateQueries({
