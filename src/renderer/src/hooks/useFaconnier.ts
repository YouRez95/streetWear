...
export const queryKeys = {
  faconniers: (page: number, limit: number, search: string) => ['faconniers', page, limit, search],
  faconniersRoot: ['faconniers'],
  activeFaconniers: (seasonId: string) => ['activeFaconniers', seasonId],
  ordersFaconnier: (seasonId: string, faconnierId: string, bonId: string) => [
    'ordersFaconnier',
    seasonId,
    faconnierId,
    bonId
  ...
  return useMutation({
    ...
    onSuccess: async (data) => {
      ...
      toast({
        ...
        description: data.message || 'Order faconnier has been created successfully.',
        variant: 'default'
      })
    },
    ...
    onSettled: (_data, _error, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.ordersFaconnier(seasonId, variables.faconnierId, ''),
        exact: false
      })
      queryClient.invalidateQueries({
        queryKey: queryKeys.faconnierSummary(seasonId, variables.faconnierId, ''),
        exact: false
      })
      queryClient.invalidateQueries({
  ...
  return useMutation({
    ...
    onSuccess: async (data) => {
      ...
      toast({
        ...
        description: data.message || 'Bon faconnier has been created successfully.',
        variant: 'default'
      })
    },
...
export function useDeleteBonFaconnier() {
  ...
  return useMutation({
    mutationFn: (bonId: string) => faconnierService.deleteBonFaconnier(bonId),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
      ...
      toast({
        ...
        description: data.message || 'Bon faconnier has been deleted successfully.',
        variant: 'default'
      })
    },
...
export function useActiveFaconniers(openBon: boolean = true, closedBon: boolean = false) {
  const { activeSeason } = useUserStore()
  return useQuery({
    queryKey: [...queryKeys.activeFaconniers(activeSeason?.id || ''), openBon, closedBon],
    queryFn: () => {
      if (!activeSeason) {
      ...
      return faconnierService.getActiveFaconniers(activeSeason.id, openBon, closedBon)
    },
    refetchOnWindowFocus: false,
    retry: false
  })
...
export function useOrdersFaconnier(faconnierId: string, bonId: string) {
  const { activeSeason } = useUserStore()
  const seasonId = activeSeason?.id || ''

  // Always call useQuery, but disable it if faconnierId or bonId is empty
  return useQuery({
    queryKey: queryKeys.ordersFaconnier(seasonId, faconnierId, bonId),
    queryFn: () => faconnierService.getOrdersFaconnier(seasonId, faconnierId, bonId),
    refetchOnWindowFocus: false,
    retry: false,
  ...
  return useMutation({
    ...
    onSuccess: async (data) => {
      ...
      toast({
        ...
        description: data.message || 'Avance faconnier has been created successfully.',
        variant: 'default'
      })
    },
    onError: (error) => showErrorToast('Error creating avance faconnier', error),
    onSettled: (_data, _error, variables) => {
      if (!variables) return
      queryClient.invalidateQueries({
        queryKey: queryKeys.ordersFaconnier(seasonId, variables.faconnierId, variables.bonId),
        exact: false
      })
      queryClient.invalidateQueries({
        queryKey: queryKeys.faconnierSummary(seasonId, variables.faconnierId, variables.bonId),
        exact: false
      })
    }
  })
...
export function useUpdateOrderQuantityReturned() {
  const queryClient = useQueryClient()
  const { activeSeason } = useUserStore()
  const seasonId = activeSeason?.id || ''
  ...
  return useMutation({
    mutationFn: ({
      ...
      faconnierId: string
      bonId: string
    }) => faconnierService.updateOrderQuantityReturned(orderId, quantityReturned),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
          title: 'Error updating order quantity returned',
          description: data.message,
          variant: 'destructive'
        })
        return
      ...
      toast({
        title: 'Order quantity returned updated successfully',
        description: data.message || 'Order quantity returned has been updated successfully.',
        variant: 'default'
      })
    },
    onError: (error) => showErrorToast('Error updating order quantity returned', error),
    onSettled: (_data, _error, variables) => {
      if (!variables) return
      queryClient.invalidateQueries({
        queryKey: queryKeys.ordersFaconnier(seasonId, variables.faconnierId, variables.bonId),
...
export function useFaconnierSummary(faconnierId: string, bonId: string) {
  ...
  return useQuery({
    ...
    queryFn: () => faconnierService.getFaconnierSummary(seasonId, faconnierId, bonId),
    refetchOnWindowFocus: false,
    retry: false,
...
export function useCreateFaconnier() {
  ...
  return useMutation({
    ...
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.faconniersRoot, exact: false })
      queryClient.invalidateQueries({
        queryKey: queryKeys.activeFaconniers(seasonId),
        exact: false
      })
...
export function useDeleteAvanceFaconnier() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      faconnierId: string
      bonId: string
    }) => faconnierService.deleteAvanceFaconnier(avanceId),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
      ...
      toast({
        ...
        description: data.message || 'Avance faconnier has been deleted successfully.',
        variant: 'default'
      })
    },
    onError: (error) => showErrorToast('Error deleting avance faconnier', error),
    onSettled: (_data, _error, variables) => {
      if (!variables) return
      queryClient.invalidateQueries({
        queryKey: queryKeys.ordersFaconnier(seasonId, variables.faconnierId, variables.bonId),
...
export function useDeleteOrderFaconnier() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      faconnierId: string
      bonId: string
    }) => faconnierService.deleteOrderFaconnier(orderId),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
      ...
      toast({
        ...
        description: data.message || 'Order faconnier has been deleted successfully.',
        variant: 'default'
      })
    },
  ...
  return useMutation({
    ...
    onSuccess: async (data) => {
      ...
      toast({
        ...
        description: data.message || 'Faconnier status has been updated successfully.',
        variant: 'default'
      })
    },
    onError: (error) => showErrorToast('Error updating Faconniers', error),
...
export function useToggleBonFaconnier() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      openBon: boolean
      closeBon: boolean
    }) => faconnierService.toggleBonFaconnier(bonId, openBon, closeBon),
    onSuccess: async (data) => {
      if (data.status === 'failed') {
        toast({
      ...
      toast({
        ...
        description: data.message || 'Bon faconnier has been opened/closed successfully.',
        variant: 'default'
      })
    },
    onError: (error) => showErrorToast('Error opening/closing bon faconnier', error),
    onSettled: () => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.activeFaconniers(seasonId),
        exact: false
