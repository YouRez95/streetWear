export const faconnierService = {
  ...
  getOrdersFaconnier: async (
    ...
    try {
      return await window.context.getOrdersFaconnier(seasonId, faconnierId, bonId, queryParams)
    } catch (error) {
  ...
  updateOrderFaconnier: async (updateOrderFaconnierData: UpdateOrderFaconnierInput) => {
    try {
      return await window.context.updateOrderFaconnier(updateOrderFaconnierData)
    } catch (error) {
      console.error('Error updating order quantity returned:', error)
      throw error
    }
  },
  deleteAvanceFaconnier: async (avanceId: string, seasonId: string) => {
    try {
      return await window.context.deleteAvanceFaconnier(avanceId, seasonId)
    } catch (error) {
      console.error('Error deleting avance faconnier:', error)
      throw error
    }
  },
  deleteOrderFaconnier: async (orderId: string, seasonId: string) => {
    try {
      return await window.context.deleteOrderFaconnier(orderId, seasonId)
    } catch (error) {
      console.error('Error deleting order faconnier:', error)
      throw error
    }
  },
  toggleBonFaconnier: async (
    bonId: string,
    seasonId: string,
    openBon: boolean,
    closeBon: boolean
    ...
    try {
      return await window.context.toggleBonFaconnier(bonId, seasonId, openBon, closeBon)
    } catch (error) {
      console.error('Error toggling bon faconnier:', error)
      throw error
    }
  },
  deleteBonFaconnier: async (bonId: string, seasonId: string) => {
    try {
      return await window.context.deleteBonFaconnier(bonId, seasonId)
    } catch (error) {
