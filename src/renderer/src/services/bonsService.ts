    ...
    autoTable(doc, {
      ...
      body: [
        [
          ...
          (order.unit_price * order.quantity).toFixed(2) + ' MAD'
        ]
      ],
...
export async function downloadDailyBon(dailyData: {
  ...
  doc.setFontSize(15)
  doc.setTextColor('#FF4C5F')
  doc.text(`Date : ${formatDateToDDMMYYYY(new Date(dailyData.date))}`, 10, 70, { align: 'left' })
  ...
  doc.text(`Quantité totale retournée: ${dailyData.totalQuantityReturned}`, 10, 147)
  ...
  if (products.length > 0) {
    ...
    autoTable(doc, {
      startY: currentY,
      head: [['Référence', 'Article', 'Quantité', 'Retourné', 'Prix Unitaire', 'Total']],
  ...
  // 2. Signature label (right side)
  doc.text('Signature:', doc.internal.pageSize.getWidth() - margin - 35, footerY + 8)
  ...
  // Output
  const pdfBuffer = doc.output('arraybuffer')
  const fileName = generateDailyFileName(dailyData.date)
  window.context.downloadBon(pdfBuffer, fileName)
}
