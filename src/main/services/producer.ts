...
export const updateOrderFaconnier: UpdateOrderFaconnier = async (updateOrderFaconnierData) => {
  const { orderId, formData, seasonId } = updateOrderFaconnierData
  try {
...
export const getOrdersFaconnier: GetOrdersFaconnier = async (
  seasonId,
  faconnierId,
  bonId,
  queryParams
  ...
  try {
    const result = await apiClient.get(
      `/api/v1/faconnier/bon/details/${seasonId}/${faconnierId}/${bonId}?page=${queryParams?.page}&limit=${queryParams?.limit}&search=${queryParams?.search}&date=${queryParams?.date}`
    )
...
export const deleteAvanceFaconnier: DeleteAvanceFaconnier = async (avanceId, seasonId) => {
  try {
...
export const deleteOrderFaconnier: DeleteOrderFaconnier = async (orderId, seasonId) => {
  try {
    const result = await apiClient.delete(`/api/v1/faconnier/orders/delete/${seasonId}/${orderId}`)
...
export const toggleBonFaconnier: ToggleBonFaconnier = async (
  bonId,
  seasonId,
  openBon,
  closeBon
...
export const deleteBonFaconnier: DeleteBonFaconnier = async (bonId, seasonId) => {
  try {
    const result = await apiClient.delete(`/api/v1/faconnier/bon/delete/${seasonId}/${bonId}`)
